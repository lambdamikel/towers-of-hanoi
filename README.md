...
A later addition reuses that same four-bit protocol to drive a **64×32 RGB LED
matrix** instead of the arm: the CP1 streams its moves to an Arduino Mega 2560
that animates the disks sliding between three pegs, rainbow-coloured by size on a
blue base. That lives under [`./matrix/`](./matrix/).

![The CP1-driven Hanoi solve on a 64×32 RGB LED matrix](./matrix/hanoi_demo.gif)

And to get programs *onto* the CP1 without typing them in, [`./cassette/`](./cassette/)
holds a small toolchain that assembles a listing and renders it as a **cassette
WAV** you play into the CP2 module — the 1983 CP1/CP2 FSK tape format,
reverse-engineered from a digitized real save and validated by a byte-exact
round-trip.
